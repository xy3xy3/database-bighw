...
class BaseModel:
    ...
    def save(self, data: dict):
        ...
        sql = f"INSERT INTO {self.table_name} ({keys}) VALUES ({values}) RETURNING *"
    ...
    def update(self, id: int, data: dict):
        ...
        sql = f"UPDATE {self.table_name} SET {set_clause} WHERE id = %(id)s RETURNING *"
        data['id'] = id
        self.cursor.execute(sql, data)
        self.conn.commit()
        return self.cursor.fetchone()
    ...
    def query(self, conditions: Optional[dict] = None):
        """查询数据"""
